...
frappe.views.Workspace = class Workspace {
    ...
    build_sidebar_section(category, root_pages) {
        ...
        root_pages.forEach((item) => {
            let is_current_page =
                frappe.router.slug(item.title) ==
                    frappe.router.slug(this.get_page_to_show().name) &&
                item.public == this.get_page_to_show().public;

            let $option = $(`<option value="${item.name}">${item.title}</option>`);
            if (is_current_page) {
        ...
        sidebar_section.append($dropdown);
